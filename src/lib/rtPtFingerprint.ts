export const RT_PT_SHA256_FINGERPRINT_PATTERN = /^sha256:[0-9a-f]{64}$/;
...
/**
 * Produces a compact, deterministic SHA-256 binding in browsers, Electron,
 * and Node without environment-specific crypto APIs.
 */
export function createRtPtSha256Fingerprint(canonicalText: string): string {
  ...
  const digest = Array.from(hash, (word) => word.toString(16).padStart(8, '0')).join('');
  return `sha256:${digest}`;
}

export function isRtPtSha256Fingerprint(value: string): boolean {
  return RT_PT_SHA256_FINGERPRINT_PATTERN.test(value);
}
