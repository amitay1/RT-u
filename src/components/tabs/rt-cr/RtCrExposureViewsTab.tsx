...
import type { RtCrExposureView, RtCrPlateSystem, RtCrSource } from '@/types/rtCr';

type ExposureViewPatch = Partial<Omit<RtCrExposureView, 'id'>>;

interface Props {
  data: RtCrExposureView[];
  source: RtCrSource;
...
export function RtCrExposureViewsTab({
  data,
  source,
  plateSystem,
  onAdd,
        ...
        {data.length === 0 ? (
          <div className="flex min-h-52 flex-col items-center justify-center rounded-2xl border border-dashed border-primary/30 bg-primary/5 px-6 py-10 text-center">
            ...
            <p className="mt-1 max-w-lg text-sm text-muted-foreground">
              Add the first required view, then define its orientation, attachment reference, geometry, IQI, exposure,
              plate loading, overlap, and identification instructions.
            </p>
                ...
                <AccordionContent className="space-y-5">
                  ...
                  <section className="space-y-3 border-t border-border/70 pt-5">
                    ...
                    <RtRadiographySetupDiagram
                      mode="cr"
                      title={`${view.viewId || `View ${index + 1}`} computed radiography setup`}
                      sourceLabel={source.sourceType === 'Gamma'
                      ...
                      receptorLabel={[plateSystem.plateDesignation, view.plateSize].filter(Boolean).join(' / ') || 'Imaging plate / cassette'}
                      viewId={view.viewId}
