...
The packaged runtime is audited before publication: `server/`, `shared/`, MRO, legacy licensing, private keys, activation packages, foreign update channels, and CAD/UT-only Node packages are forbidden, while `express`, `electron-updater`, and `electron-log` remain available to the Electron main process.
